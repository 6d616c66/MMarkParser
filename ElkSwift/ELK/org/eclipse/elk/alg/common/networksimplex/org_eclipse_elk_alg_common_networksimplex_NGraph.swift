...
internal class NGraph {
     ...
     */
    internal func writeDebugGraph(filePath: String) {
     ...
     */
    internal func makeConnected() -> NNode? {
        var id = 0
        for n in nodes {
            n.internalId = id
            id += 1
     ...
     */
    internal func createArtificialRootAndConnect(nodesToConnect: [NNode]) -> NNode {
     ...
     */
    internal func findConCompRepresentatives() -> [NNode] {
        var ccRep = [NNode]()
    ...
    internal func dfs(node: NNode, mark: inout [Bool]) {
        if mark[node.internalId] {
            return
        }
