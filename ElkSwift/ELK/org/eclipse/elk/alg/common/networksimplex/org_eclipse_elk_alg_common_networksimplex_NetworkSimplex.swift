 ...
 */
internal final class NetworkSimplex {
    ...
    internal var previousLayeringNodeCounts: [Int]?
    /** Whether to apply `balance(int[])`. */
    internal var balance = false
    /** A limit on the number of iterations. */
    internal var iterationLimit = Int.max
    /** Empirically determined threshold when removing subtrees pays off. */
    internal static let REMOVE_SUBTREES_THRESH = 40
     ...
     */
    internal var subtreeNodesStack: [(node: NNode, edge: NEdge)]?
     ...
     */
    internal static func forGraph(_ graph: NGraph) -> NetworkSimplex {
     ...
     */
    internal func withBalancing(_ doBalance: Bool) -> NetworkSimplex {
     ...
     */
    internal func withPreviousLayering(_ considerPreviousLayering: [Int]?) -> NetworkSimplex {
    ...
    /**
     ...
     - Returns: the `NetworkSimplex` instance for further configuration or execution.
     */
    internal func withIterationLimit(_ limit: Int) -> NetworkSimplex {
     ...
     */
    internal func execute(_ monitor: IElkProgressMonitor) {
     ...
     */
    internal func reattachSubtrees() {
        guard let graph = graph else { return }
     ...
     */
    internal func layeringTopologicalNumbering(_ initialRootNodes: [NNode]) {
        guard let graph = graph else { return }
     ...
     */
    internal func minimalSpan(_ node: NNode) -> (Int, Int) {
     ...
     */
    internal func tightTreeDFS(_ node: NNode) -> Int {
        var nodeCount = 1
        node.treeNode = true
        for edge in node.connectedEdges {
     ...
     */
    internal func minimalSlack() -> NEdge? {
     ...
     */
    @discardableResult
    internal func postorderTraversal(_ node: NNode) -> Int {
        var lowest = Int.max
        for edge in node.connectedEdges {
     ...
     */
    internal func isInHead(_ node: NNode, _ edge: NEdge) -> Bool {
     ...
     */
    internal func enterEdge(_ leave: NEdge) -> NEdge {
        if !leave.treeEdge {
    ...
    /**
     ...
     - SeeAlso: `leaveEdge()`
     */
    internal func exchange(leave: NEdge, enter: NEdge) {
        if !leave.treeEdge {
     ...
     */
    internal func balance(_ filling: [Int]) {
        guard let graph = graph else { return }
