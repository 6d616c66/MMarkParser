 ...
 */
internal class GridContainerCell: ContainerCell {

    // MARK: - Constants

    internal static let ROWS = ContainerArea.count
    internal static let COLUMNS = ROWS

    // MARK: - Properties

    internal let gap: Double
    internal let tabular: Bool
    internal let symmetrical: Bool
    internal var cells: [[Cell?]] = Array(repeating: Array(repeating: nil, count: 3), count: 3)

    internal var centerCellMinimumSize: KVector?
    internal var onlyCenterCellContributesToMinimumSize = false
    internal var centerCellRect = ElkRectangle()
    ...
    internal init(tabular: Bool, symmetrical: Bool, gap: Double) {
    ...
    internal convenience init(_ tabular: Bool, _ symmetrical: Bool, _ gap: Double) {
        self.init(tabular: tabular, symmetrical: symmetrical, gap: gap)
    }

    // MARK: - Getters / Setters

    internal func getGap() -> Double {
        return gap
    }

    internal func getCell(row: ContainerArea, col: ContainerArea) -> Cell? {
        return cells[row.ordinal][col.ordinal]
    }

    /// Convenience: getCell(.BEGIN, .END)
    internal func getCell(_ row: ContainerArea, _ col: ContainerArea) -> Cell? {
        return cells[row.ordinal][col.ordinal]
    }

    internal func setCell(row: ContainerArea, col: ContainerArea, cell: Cell?) {
        cells[row.ordinal][col.ordinal] = cell
    }

    /// Convenience: setCell(row, col, cell) positional
    internal func setCell(_ row: ContainerArea, _ col: ContainerArea, _ cell: Cell?) {
        cells[row.ordinal][col.ordinal] = cell
    }

    internal func setCenterCellMinimumSize(_ minimumSize: KVector) {
        self.centerCellMinimumSize = KVector(x: minimumSize.x, y: minimumSize.y)
    }

    internal func setOnlyCenterCellContributesToMinimumSize(_ contribution: Bool) {
        self.onlyCenterCellContributesToMinimumSize = contribution
    }

    internal func getCenterCellRectangle() -> ElkRectangle {
    ...
    internal override func layoutChildrenHorizontally() {
        if tabular {
    ...
    internal override func layoutChildrenVertically() {
    ...
    internal func minColumnWidths(row: ContainerArea?, respectContributionFlag: Bool) -> [Double] {
        var colWidths = [
    ...
    internal func minWidthOfColumn(column: ContainerArea, row: ContainerArea?, respectContributionFlag: Bool) -> Double {
        var maxMinWidth = 0.0
    ...
    internal func minRowHeights(respectContributionFlag: Bool) -> [Double] {
        var rowHeights = [
    ...
    internal func minHeightOfRow(row: ContainerArea, respectContributionFlag: Bool) -> Double {
    ...
    internal func sumWithGaps(_ values: [Double]) -> Double {
        var sum = 0.0
    ...
    internal func applyWidthsToRow(row: ContainerArea, colWidths: [Double]) {
        let cellRectangle = getCellRectangle()
        let cellPadding = getPadding()
    ...
    internal func applyWidthToColumn(column: ContainerArea, x: Double, colWidths: [Double]) {
        for row in 0..<GridContainerCell.ROWS {
            applyHorizontalLayout(cells[row][column.ordinal], x: x, width: colWidths[column.ordinal])
        }
    }

    internal func applyHeightToRow(row: ContainerArea, y: Double, rowHeights: [Double]) {
        for column in 0..<GridContainerCell.COLUMNS {
