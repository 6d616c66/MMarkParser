 ...
 */
internal class StripContainerCell: ContainerCell {

    // MARK: - Properties

    /** Whether we lay children out in rows or columns. */
    internal let containerMode: Strip
    ...
    internal init(mode: Strip, symmetrical: Bool, gap: Double) {
    ...
    internal func getContainerMode() -> Strip {
    ...
    internal func getCell(_ area: ContainerArea) -> Cell? {
        return cells[area.rawValue]
    }

    internal func setCell(_ area: ContainerArea, cell: Cell?) {
        cells[area.rawValue] = cell
    }

    /// Convenience: setCell(area, cell) without label
    internal func setCell(_ area: ContainerArea, _ cell: Cell?) {
        cells[area.rawValue] = cell
    }
    ...
    internal func minCellWidths(respectContributionFlag: Bool) -> [Double] {
    ...
    internal func minCellHeights(respectContributionFlag: Bool) -> [Double] {
    ...
    internal enum Strip: Int, CaseIterable {
        case VERTICAL = 0
        case HORIZONTAL = 1
    }

    /// Convenience: setPadding for compatibility
    internal func setPadding(_ newPadding: ElkPadding) {
        padding = newPadding
    }
}
