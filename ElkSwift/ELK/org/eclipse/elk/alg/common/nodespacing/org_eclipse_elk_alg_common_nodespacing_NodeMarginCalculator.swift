 ...
 */
internal final class NodeMarginCalculator {

    internal var includeLabels = true
    internal var includePorts = true
    internal var includePortLabels = true
    internal var includeEdgeHeadTailLabels = true

    internal let adapter: GraphAdapter

    internal init(_ adapter: GraphAdapter) {
    ...
    @discardableResult
    internal func excludeLabels() -> NodeMarginCalculator {
        self.includeLabels = false
        return self
    }

    @discardableResult
    internal func excludePorts() -> NodeMarginCalculator {
        self.includePorts = false
        return self
    }

    @discardableResult
    internal func excludePortLabels() -> NodeMarginCalculator {
        self.includePortLabels = false
        return self
    }

    @discardableResult
    internal func excludeEdgeHeadTailLabels() -> NodeMarginCalculator {
    ...
    internal func process(node: NodeAdapter) {
        let spacing: Double = adapter.getProperty(CoreOptions.SPACING_LABEL_NODE) ?? 0.0
        process(node: node, spacing: spacing)
    }

    internal func process(node: NodeAdapter, spacing labelSpacing: Double) {
    ...
    internal func processEdgeHeadTailLabels(
