...
internal class ComponentsToCGraphTransformer {

    private var _cGraph: CGraph?
    internal var spacing: Double

    // internal mappings
    internal var oldPosition: [ObjectIdentifier: KVector] = [:]
    internal var offsets: [ObjectIdentifier: CRectNode] = [:]
    ...
    internal var externalExtensions: [String: (first: Direction, second: CNode)] = [:]
    internal var externalPlaceholder: [Direction: [Pair<CGroup, CNode>]] = [:]

    internal init(spacing: Double) {
        self.spacing = spacing
    }

    internal func getOffset(_ c: InternalComponent) -> KVector {
    ...
    internal func getGlobalOffset() -> KVector {
    ...
    internal func transform(_ ccs: InternalConnectedComponents) -> CGraph {
    ...
    internal func setLock(_ cNode: CNode, portSides: Set<PortSide>) {
    ...
    internal class CRectNode: CNode {
        internal var rect: ElkRectangle
        internal var individualSpacing: Double?
        ...
        internal func getPositionVector() -> KVector {
            return KVector(x: rect.x, y: rect.y)
        }

        internal override func getHorizontalSpacing() -> Double {
            return individualSpacing ?? 0
        }

        internal override func getVerticalSpacing() -> Double {
            return individualSpacing ?? 0
        }

        internal override func applyElementPosition() {
            rect.x = hitbox.x
            rect.y = hitbox.y
        }

        internal override func getElementPosition() -> Double {
            return rect.x
        }
    }
}
