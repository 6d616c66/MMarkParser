 ...
 */
internal final class OneDimensionalCompactor {

    /** Longest path-based compaction strategy. */
    internal static let LONGEST_PATH_COMPACTION: ICompactionAlgorithm = LongestPathCompaction()

    /** Currently used compaction algorithm. */
    internal var compactionAlgorithm: ICompactionAlgorithm = OneDimensionalCompactor.LONGEST_PATH_COMPACTION

    /** Constraint calculation using a scanline technique. */
    internal static let SCANLINE_CONSTRAINTS: IConstraintCalculationAlgorithm = ScanlineConstraintCalculator()
    /** Constraint calculation by pair-wise comparison of CNodes. */
    internal static let QUADRATIC_CONSTRAINTS: IConstraintCalculationAlgorithm = QuadraticConstraintCalculation()
    /** Currently used instance of the constraint calculation algorithm. */
    internal var constraintAlgorithm: IConstraintCalculationAlgorithm = OneDimensionalCompactor.SCANLINE_CONSTRAINTS

    /** the CGraph. */
    internal var cGraph: CGraph
    /** compacting in this direction. */
    internal var direction: Direction = Direction.UNDEFINED
    /** a function that sets the CNode#reposition flag according to the direction. */
    internal var lockingStrategy: ((Pair<CNode, Direction>) -> Bool)?
    /** flag indicating whether the finish() method has been called. */
    internal var finished: Bool = false

    internal init(_ cGraph: CGraph) {
        self.cGraph = cGraph
    ...
    @discardableResult
    internal func setLockingStrategy(_ strategy: @escaping (Pair<CNode, Direction>) -> Bool) -> OneDimensionalCompactor {
        lockingStrategy = strategy
        return self
    }

    @discardableResult
    internal func compact() -> OneDimensionalCompactor {
        if finished {
    ...
    @discardableResult
    internal func finish() -> OneDimensionalCompactor {
        _ = changeDirection(.LEFT)
        finished = true
        return self
    }
    ...
    @discardableResult
    internal func changeDirection(_ dir: Direction) -> OneDimensionalCompactor {
        if finished {
            assertionFailure("The compactor instance has been finished already.")
            return self
        }
    ...
    @discardableResult
    internal func applyLockingStrategy() -> OneDimensionalCompactor {
        return applyLockingStrategy(direction)
    }

    @discardableResult
    internal func applyLockingStrategy(_ dir: Direction) -> OneDimensionalCompactor {
    ...
    @discardableResult
    internal func forceConstraintsRecalculation() -> OneDimensionalCompactor {
        calculateConstraints()
        return self
    }

    @discardableResult
    internal func calculateGroupOffsets() -> OneDimensionalCompactor {
    ...
    internal func isLocked(_ node: CNode, _ dir: Direction) -> Bool {
        return node.lock.get(direction: dir)
    }

    internal func isLocked(_ group: CGroup, _ dir: Direction) -> Bool {
    ...
    internal func transposeHitboxes() {
        for cNode in cGraph.cNodes {
    ...
    internal func calculateConstraintsForCGroups() {
        for group in cGraph.cGroups {
            group.outDegree = 0
    ...
    internal func reverseConstraints() {
