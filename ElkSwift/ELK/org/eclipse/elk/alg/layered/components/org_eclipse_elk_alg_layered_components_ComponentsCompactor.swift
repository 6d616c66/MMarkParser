...
internal class ComponentsCompactor {

    internal var compactor: OneDimensionalComponentsCompaction<LNode, Set<LEdge>>?

    internal var yetAnotherOffset = KVector()
    internal var compactedGraphSize = KVector()

    internal var graphTopLeft = KVector(x: Double.greatestFiniteMagnitude, y: Double.greatestFiniteMagnitude)
    internal var graphBottomRight = KVector(x: -Double.greatestFiniteMagnitude, y: -Double.greatestFiniteMagnitude)

    internal static let EPSILON: Double = 0.0001

    internal var offset: KVector { return yetAnotherOffset }
    internal var graphSize: KVector { return compactedGraphSize }

    internal func getOffset() -> KVector {
        return yetAnotherOffset
    }

    internal func getGraphSize() -> KVector {
        return compactedGraphSize
    }

    internal func compact(_ graphs: [LGraph], _ originalGraphsSize: KVector, _ spacing: Double) {
    ...
    internal func transformLGraph(_ graph: LGraph) -> InternalComponent {
    ...
    internal func createDummyNode(_ graph: LGraph) -> LNode {
    ...
    internal func componentHullPoints(_ graph: LGraph) -> Hullpoints {
        let pts = Hullpoints()
    ...
    internal func addLGraphElementBounds(_ pts: Hullpoints, _ element: LShape, _ offset: KVector) {
        var margins = LMargin()
    ...
    internal func isExternalEdge(_ edge: LEdge) -> Bool {
        return edge.source?.node?.type == .EXTERNAL_PORT || edge.target?.node?.type == .EXTERNAL_PORT
    }

    internal func transformLEdge(_ externalEdge: LEdge, hullPoints: Hullpoints, outerSegments: inout OuterSegments) -> InternalExternalExtension {
    ...
    internal func segmentToRectangle(_ p1: KVector, _ p2: KVector, _ extent: Double) -> ElkRectangle {
    ...
    internal func edgeToSegments(_ edge: LEdge, externalExtension: InternalExternalExtension) -> Segments {
    ...
    internal func getExternalPortPosition(_ pos: KVector, _ ps: PortSide) -> KVector {
        switch ps {
        case .NORTH:
    ...
    internal func getPortPositionOnMargin(_ port: LPort) -> KVector {
    ...
    internal func portSideToDirection(_ side: PortSide) -> Direction {
        switch side {
        case .NORTH:
            return .UP
        case .WEST:
...
internal class InternalConnectedComponents {
    internal var components: [InternalComponent] = []
    internal var containsExternalPorts = false

    internal func getComponents() -> [InternalComponent] {
        return components
    }

    internal func isContainsExternalExtensions() -> Bool {
...
internal class InternalComponent {
    ...
    internal var containsRegularNodes: Bool

    internal var rectilinearConvexHull: [ElkRectangle] = []
    internal var externalExtensions: [InternalUnionExternalExtension] = []

    internal init(graph: LGraph) {
        self.graph = graph
    ...
    internal var externalExtensionSides: Set<PortSide> {
        return graph.getProperty(InternalProperties.EXT_PORT_CONNECTIONS) ?? []
    }

    internal func getHull() -> [ElkRectangle] {
        return rectilinearConvexHull
    }

    internal func getExternalExtensions() -> [InternalUnionExternalExtension] {
    ...
    internal func getExternalEdges() -> [LEdge] {
        var edges: [LEdge] = []
...
internal class InternalUnionExternalExtension {
    internal var edges: Set<LEdge> = []
    internal var side: PortSide = .UNDEFINED
    internal var extensionRect: ElkRectangle = ElkRectangle()
    internal var placeholderRect: ElkRectangle?

    internal init() {}

    internal func getRepresentative() -> Set<LEdge> {
        return edges
    }

    internal func getRepresentor() -> ElkRectangle {
        return extensionRect
    }

    internal func getDirection() -> Direction {
        switch side {
    ...
    internal func getPlaceholder() -> ElkRectangle? {
        return placeholderRect
    }
}

internal class InternalExternalExtension {
    internal var edge: LEdge
    internal var externalPort: LPort
    internal var externalPortSide: PortSide

    internal var externalExtension: ElkRectangle = ElkRectangle()
    internal var parent: ElkRectangle = ElkRectangle()

    internal init(edge: LEdge) {
        self.edge = edge
...
internal class Hullpoints {
    internal var points: [Point] = []
    internal var topLeft = KVector(x: Double.greatestFiniteMagnitude, y: Double.greatestFiniteMagnitude)
    internal var bottomRight = KVector(x: -Double.greatestFiniteMagnitude, y: -Double.greatestFiniteMagnitude)

    internal func addPoint(_ newElement: Point) {
    ...
    internal func addKVector(_ vector: KVector) {
        addPoint(Point(x: vector.x, y: vector.y))
    }

    internal func addRect(_ rect: ElkRectangle) {
...
internal class Segments {
    internal var innerSegments: [(first: KVector, second: KVector)] = []
    internal var outerSegment: (first: KVector, second: KVector) = (KVector(), KVector())
}

internal class OuterSegments {
    internal var minValues: [Double] = Array(repeating: Double.greatestFiniteMagnitude, count: 5)
    internal var maxValues: [Double] = Array(repeating: -Double.greatestFiniteMagnitude, count: 5)
    internal var extent: [Double] = Array(repeating: -Double.greatestFiniteMagnitude, count: 5)
}
