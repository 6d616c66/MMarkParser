 ...
 */
internal final class ComponentsProcessor {

    /** Cached instance of a ComponentGroupGraphPlacer. */
    internal let componentGroupGraphPlacer = ComponentGroupGraphPlacer()
    /** Cached instance of a ComponentGroupModelOrderGraphPlacer. */
    internal let componentGroupModelOrderGraphPlacer = ComponentGroupModelOrderGraphPlacer()
    /** Cached instance of a ModelOrderRowGraphPlacer. */
    internal let modelOrderRowGraphPlacer = ModelOrderRowGraphPlacer()
    /** Cached instance of a SimpleRowGraphPlacer. */
    internal let simpleRowGraphPlacer = SimpleRowGraphPlacer()
    /** Graph placer to be used to combine the different components back into a single graph. */
    internal lazy var graphPlacer: AbstractGraphPlacer = simpleRowGraphPlacer

    /**
     * Split the given graph into its connected components.
     */
    internal func split(_ graph: LGraph) -> [LGraph] {
        var result: [LGraph] = []
     ...
     */
    internal func dfs(_ node: LNode, data: Pair<Set<PortSide>, [LNode]>?) -> Pair<Set<PortSide>, [LNode]>? {
     ...
     */
    internal func combine(_ components: [LGraph], target: LGraph) {
