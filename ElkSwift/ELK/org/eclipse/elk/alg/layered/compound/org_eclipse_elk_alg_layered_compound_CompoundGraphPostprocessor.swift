 ...
 */
internal class CompoundGraphPostprocessor: ILayoutProcessor {
    internal typealias G = LGraph

    /**
     * A predicate that checks if a given cross hierarchy edge has junction points.
     */
    internal static let hasJunctionPointsPredicate: (CrossHierarchyEdge) -> Bool = { chEdge in
    ...
    internal func process(_ graph: LGraph, _ monitor: IElkProgressMonitor) {
     ...
     */
    internal func clearJunctionPoints(_ origEdge: LEdge, crossHierarchyEdges: [CrossHierarchyEdge]) -> KVectorChain? {
     ...
     */
    internal func copyJunctionPoints(_ source: LEdge, target: KVectorChain, offset: KVector) {
     ...
     */
    internal func copyLabelsBack(hierarchySegment: LEdge, origEdge: LEdge, referenceGraph: LGraph) {
