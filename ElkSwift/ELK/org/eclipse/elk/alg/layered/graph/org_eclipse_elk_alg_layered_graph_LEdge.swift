 ...
 */
internal final class LEdge: LGraphElement {

    internal var bendPoints = KVectorChain()
    internal var source: LPort?
    internal var target: LPort?
    internal var labels: [LLabel] = []

    internal func reverse(_ layeredGraph: LGraph, _ adaptPorts: Bool) {
    ...
    internal func getSource() -> LPort? {
        return source
    }

    internal func setSource(_ source: LPort?) {
    ...
    internal func getTarget() -> LPort? {
        return target
    }

    internal func setTarget(_ target: LPort?) {
    ...
    internal func setTargetAndInsertAtIndex(_ targetPort: LPort?, _ index: Int) {
        if let oldTarget = self.target {
            oldTarget.incomingEdges.removeAll { $0 === self }
        }
    ...
    internal func isInLayerEdge() -> Bool {
        return !isSelfLoop() && (source?.getNode()?.getLayer() === target?.getNode()?.getLayer())
    }

    internal func getBendPoints() -> KVectorChain {
        return bendPoints
    }
    ...
    internal func getOther(_ port: LPort) -> LPort {
    ...
    internal func getOther(_ node: LNode) -> LNode {
