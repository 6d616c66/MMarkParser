...
internal final class LGraphUtil {
    
    private init() { }
    
    internal static func toNodeArray(_ nodes: [LNode]) -> [LNode] {
        return nodes
    }
    
    internal static func toEdgeArray(_ edges: [LEdge]) -> [LEdge] {
        return edges
    }
    
    internal static func toPortArray(_ ports: [LPort]) -> [LPort] {
        return ports
    }
    
    // MARK: - Node Resizing
    
    internal static func resizeNode(_ node: LNode, newSize: KVector, movePorts: Bool, moveLabels: Bool) {
    ...
    internal static func offsetGraphs(_ graphs: [LGraph], offsetx: Double, offsety: Double) {
        for graph in graphs {
            offsetGraph(graph, offsetx: offsetx, offsety: offsety)
        }
    }
    
    internal static func offsetGraph(_ graph: LGraph, offsetx: Double, offsety: Double) {
    ...
    internal static func offsetGraph(_ graph: LGraph, dx: Double, dy: Double) {
        offsetGraph(graph, offsetx: dx, offsety: dy)
    }

    // MARK: - Layer Things
    
    internal static func placeNodesHorizontally(_ layer: Layer, xoffset: Double) {
    ...
    internal static func findMaxNonDummyNodeWidth(_ layer: Layer, respectNodeMargins: Bool) -> Double {
    ...
    internal static func computeGraphProperties(_ layeredGraph: LGraph) {
    ...
    internal static func createPort(_ node: LNode, _ endPoint: KVector?, _ type: PortType, _ layeredGraph: LGraph) -> LPort {
        var port: LPort
        let direction = getDirection(layeredGraph)
    ...
    internal static func calcPortSide(_ port: LPort, direction: Direction) -> PortSide {
    ...
    internal static func calcPortOffset(_ port: LPort, side: PortSide) -> Double {
    ...
    internal static func centerPoint(_ point: KVector, boundary: KVector, side: PortSide) {
        switch side {
        case .NORTH:
    ...
    internal static func provideCollectorPort(_ layeredGraph: LGraph, node: LNode, type: PortType, side: PortSide) -> LPort {
        var port: LPort?
    ...
    internal static func initializePort(_ port: LPort, _ portConstraints: PortConstraints, _ direction: Direction, _ anchorPos: KVector?) {
    ...
    internal static func createExternalPortDummy(_ propertyHolder: IPropertyHolder,
    ...
    internal static func getExternalPortPosition(_ graph: LGraph, _ portDummy: LNode, _ portWidth: Double, _ portHeight: Double) -> KVector {
    ...
    internal static func isDescendant(_ child: LNode?, _ parent: LNode?) -> Bool {
    ...
    internal static func changeCoordSystem(_ point: KVector, oldGraph: LGraph, newGraph: LGraph) {
    ...
    internal static func getIndividualOrInherited<T>(_ node: LNode, property: IProperty) -> T {
        var result: T?
    ...
    internal static func getDirection(_ graph: LGraph) -> Direction {
    ...
    internal static func getMinimalModelOrder(_ graph: LGraph) -> Int {
        var order = Int.max
        for node in graph.getLayerlessNodes() {
...
extension Layer: Sequence {
    internal func makeIterator() -> IndexingIterator<[LNode]> {
        return getNodes().makeIterator()
    }
}

extension LNode {
    internal convenience init(layeredGraph: LGraph) {
