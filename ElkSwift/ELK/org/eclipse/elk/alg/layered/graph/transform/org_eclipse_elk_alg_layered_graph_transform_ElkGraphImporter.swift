...
internal class EcoreUtil {
    internal static func getRootContainer(_ obj: AnyObject) -> AnyObject {
    ...
    internal static func isAncestor(_ ancestor: AnyObject?, _ obj: AnyObject?) -> Bool {
...
internal class ElkGraphImporter {

    internal init() {}

    internal var nodeAndPortMap: [ObjectIdentifier: LGraphElement] = [:]

    private func mapKey(_ element: AnyObject) -> ObjectIdentifier {
        return ObjectIdentifier(element)
    }

    internal subscript(element: AnyObject) -> LGraphElement? {
    ...
    internal func importGraph(_ elkgraph: ElkNode) throws -> LGraph {
    ...
    internal func ensureDefinedPortSide(_ lgraph: LGraph, _ elkport: ElkPort) {
    ...
    internal func shouldCalculateMinimumGraphSize(_ elkgraph: ElkNode) -> Bool {
        return !(elkgraph.getProperty(LayeredOptions.NODE_SIZE_CONSTRAINTS) as? SizeConstraint ?? []).isEmpty
    }
    
    internal func calculateMinimumGraphSize(_ elkgraph: ElkNode, _ lgraph: LGraph) {
    ...
    internal func importFlatGraph(_ elkgraph: ElkNode, _ lgraph: LGraph) throws {
        var index = 0
    ...
    internal func importHierarchicalGraph(_ elkgraph: ElkNode, _ lgraph: LGraph) throws {
    ...
    internal func needsModelOrder(_ child: ElkNode) -> Bool {
    ...
    internal func needsModelOrderBasedOnParent(_ elkgraph: ElkNode) -> Bool {
    ...
    internal func hasInsideSelfLoops(_ elknode: ElkNode) -> Bool {
    ...
    internal func findCoordinateSystemOrigin(_ elkedge: ElkEdge, _ topLevelElkGraph: ElkNode, _ topLevelLGraph: LGraph) -> LGraph? {
        let source = ElkGraphUtil.connectableShapeToNode(elkedge.sources[0])
        let target = ElkGraphUtil.connectableShapeToNode(elkedge.targets[0])
    ...
    internal func createLGraph(_ elkgraph: ElkNode) -> LGraph {
        let lgraph = LGraph()
    ...
    internal func checkExternalPorts(_ elkgraph: ElkNode, _ graphProperties: inout Set<GraphProperties>) {
    ...
    internal func transformExternalPort(_ elkgraph: ElkNode, _ lgraph: LGraph, _ elkport: ElkPort) {
    ...
    internal func calculateNetFlow(_ elkport: ElkPort) -> Int {
    ...
    internal func isConnectedToExternalNodes(_ elkport: ElkPort) -> Bool {
    ...
    @discardableResult
    internal func transformNode(_ elknode: ElkNode, _ lgraph: LGraph) -> LNode {
    ...
    @discardableResult
    internal func transformPort(_ elkport: ElkPort, _ parentLNode: LNode,
    ...
    @discardableResult
    internal func transformEdge(_ elkedge: ElkEdge, _ elkparent: ElkNode, _ lgraph: LGraph) throws -> LEdge {
        try checkEdgeValidity(elkedge)
    ...
    internal func checkEdgeValidity(_ edge: ElkEdge) throws {
    ...
    internal func transformLabel(_ elklabel: ElkLabel) -> LLabel {
