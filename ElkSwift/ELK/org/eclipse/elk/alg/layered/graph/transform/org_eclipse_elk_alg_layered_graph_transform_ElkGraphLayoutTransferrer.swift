...
internal let ZERO_OFFSET = KVector(x: 0.0, y: 0.0)

// MARK: - Main Class

internal class ElkGraphLayoutTransferrer {
    
    internal func apply(_ lgraph: LGraph) {
    ...
    internal func applyNodeLayout(_ lnode: LNode, offset: KVector) {
    ...
    internal func applyEdgeLayout(_ ledge: LEdge, routing: EdgeRouting, offset: KVector, additionalPadding: LPadding) {
    ...
    internal func calculateHierarchicalOffset(_ ledge: LEdge) -> KVector {
    ...
    internal func applyParentNodeLayout(_ lgraph: LGraph) {
    ...
    internal static func collectNodes(_ lgraph: LGraph) -> [LNode] {
        var nodes: [LNode] = []
    ...
    internal static func applyNodeLayout(_ lnode: LNode, elknode: ElkNode, offset: KVector) {
        let nodeID = lnode.getProperty(LayeredOptions.CROSSING_MINIMIZATION_POSITION_ID)
        let layerID = lnode.getProperty(LayeredOptions.LAYERING_LAYER_ID)
        elknode.setProperty(LayeredOptions.CROSSING_MINIMIZATION_POSITION_ID, nodeID)
        elknode.setProperty(LayeredOptions.LAYERING_LAYER_ID, layerID)
    ...
    internal static func applyEdgeLayout(_ ledge: LEdge, elkedge: ElkEdge, routing: EdgeRouting, offset: KVector, additionalPadding: LPadding) {
    ...
    internal static func calculateHierarchicalOffsetNonMutating(_ ledge: LEdge) -> KVector {
        guard let targetCoordinateSystem = ledge.getProperty(InternalProperties.COORDINATE_SYSTEM_ORIGIN) as? LGraph else {
            return ZERO_OFFSET
        }
    ...
    internal static func applyParentNodeLayout(_ lgraph: LGraph, _ elknode: ElkNode) {
        let sizeConstraints = elknode.getProperty(LayeredOptions.NODE_SIZE_CONSTRAINTS) as? SizeConstraint ?? []
        let sizeConstraintsIncludedPortLabels = sizeConstraints.contains(.portLabels)
