...
internal class org_eclipse_elk_alg_layered_intermediate_preserveorder_ModelOrderNodeComparator {
    internal var previousLayer: [LNode] = []
    internal var graph: LGraph
    internal let orderingStrategy: OrderingStrategy
    internal let groupOrderStrategy: GroupOrderStrategy
    ...
    internal var longEdgeNodeOrder: LongEdgeOrderingStrategy = .EQUAL
    internal var beforePorts: Bool
    ...
    internal func getModelOrderFromConnectedEdges(_ n: LNode) -> Int {
    ...
    internal func getFirstIncomingPortOfNode(
        _ node: LNode
    ) -> LPort? {
    ...
    internal func getFirstOutgoingPortOfNode(
        _ node: LNode
    ) -> LPort? {
    ...
    internal func id(_ node: LNode) -> ObjectIdentifier {
        ObjectIdentifier(node)
    }

    internal func ensureNodeInMaps(_ node: LNode) {
        let nodeId = id(node)
