...
internal class org_eclipse_elk_alg_layered_intermediate_wrapping_GraphStats {

    // MARK: - Property keys
    internal static let DIRECTION_KEY = "org.eclipse.elk.direction"
    internal static let ASPECT_RATIO_KEY = "org.eclipse.elk.aspectRatio"
    internal static let WRAPPING_CORRECTION_FACTOR_KEY = "org.eclipse.elk.layered.wrapping.correctionFactor"
    internal static let SPACING_NODE_NODE_BETWEEN_LAYERS_KEY = "org.eclipse.elk.layered.spacing.nodeNodeBetweenLayers"
    internal static let SPACING_NODE_NODE_KEY = "org.eclipse.elk.spacing.nodeNode"
    internal static let WRAPPING_VALIDIFY_FORBIDDEN_INDICES_KEY = "org.eclipse.elk.layered.wrapping.validify.forbiddenIndices"

    // MARK: - Public fields (computed during instantiation)
    internal let graph: LGraph
    internal let dar: Double
    internal let longestPath: Int

    // MARK: - Private fields
    internal var spacing: Double = 0
    internal var inLayerSpacing: Double = 0

    // Computed on demand
    internal var maxWidth: Double?
    internal var maxHeight: Double?
    internal var sumWidth: Double?

    internal var _widths: [Double]?
    internal var _heights: [Double]?

    internal var cutsAllowed: [Bool]?
    ...
    internal func getMaxWidth() -> Double {
    ...
    internal func getSumWidth() -> Double {
    ...
    internal func getWidths() -> [Double] {
    ...
    internal func getMaxHeight() -> Double {
    ...
    internal func getHeights() -> [Double] {
    ...
    internal func isCutAllowed(_ layerIndex: Int) -> Bool {
        if cutsAllowed == nil {
            initCutAllowed()
        }
    ...
    internal func getCutsAllowed() -> [Bool] {
        if cutsAllowed == nil {
            initCutAllowed()
        }
        let cached = cutsAllowed ?? []
    ...
    internal func initWidthsAndHeights() {
        let n = longestPath
    ...
    internal func determineWidth(_ fun: (Double, Double) -> Double) -> Double {
        let layers = graph.getLayers()
    ...
    internal func determineLayerWidth(_ l: Layer) -> Double {
        var maxW: Double = 0
        for n in l.getNodes() {
    ...
    internal func determineChunkWidth(_ lowIdx: Int, _ highIdx: Int) -> Double {
        let layers = graph.getLayers()
        var sum: Double = 0
    ...
    internal func determineHeight(_ fun: (Double, Double) -> Double) -> Double {
        let layers = graph.getLayers()
        guard let first = layers.first else { return 0 }
    ...
    internal func determineLayerHeight(_ layer: Layer) -> Double {
        var lH: Double = 0
    ...
    internal func determineChunkHeight(_ lowIdx: Int, _ highIdx: Int) -> Double {
        let layers = graph.getLayers()
        var maxH: Double = 0
        for i in lowIdx..<highIdx {
    ...
    internal func isCutAllowedForLayer(_ layer: Layer) -> Bool {
