...
internal class ElkLayered {

    // MARK: - Variables

    internal let graphConfigurator = GraphConfigurator()
    internal let componentsProcessor = ComponentsProcessor()
    internal let compoundGraphPreprocessor = CompoundGraphPreprocessor()
    internal let compoundGraphPostprocessor = CompoundGraphPostprocessor()
    internal var testController: TestController? = nil

    // MARK: - Regular Layout

    internal func doLayout(_ lgraph: LGraph, _ monitor: IElkProgressMonitor?) {
    ...
    internal func doCompoundLayout(_ lgraph: LGraph, _ monitor: IElkProgressMonitor?) {
        let theMonitor: IElkProgressMonitor = monitor ?? BasicProgressMonitor()
    ...
    internal func hierarchicalLayout(_ lgraph: LGraph, _ monitor: IElkProgressMonitor) {
    ...
    internal func collectAllGraphsBottomUp(_ root: LGraph) -> [LGraph] {
    ...
    internal func reviewAndCorrectHierarchicalProcessors(_ root: LGraph, _ graphs: [LGraph]) {
    ...
    internal func isRoot(_ graph: LGraph) -> Bool {
        return graph.getParentNode() == nil
    }

    internal func hasNestedGraph(_ node: LNode) -> Bool {
    ...
    internal class TestExecutionState {
        ...
        internal func getGraphs() -> [LGraph] {
            return graphs
        }

        internal func getStep() -> Int {
            return step
        }
    }

    internal func prepareLayoutTest(_ lgraph: LGraph) -> TestExecutionState {
    ...
    internal func isLayoutTestFinished(_ state: TestExecutionState) -> Bool {
    ...
    internal func runLayoutTestUntil(_ phase: AnyClass, _ inclusive: Bool, _ state: TestExecutionState) {
        guard let algorithm = state.graphs.first?.getProperty(InternalProperties.PROCESSORS) as? [AnyGraphProcessor] else {
            return
        }
    ...
    internal func runLayoutTestUntil(_ phase: AnyClass, _ state: TestExecutionState) {
        runLayoutTestUntil(phase, true, state)
    }

    internal func runLayoutTestStep(_ state: TestExecutionState) {
    ...
    internal func getLayoutTestConfiguration(_ state: TestExecutionState) -> [AnyGraphProcessor] {
        return state.graphs.first?.getProperty(InternalProperties.PROCESSORS) as? [AnyGraphProcessor] ?? []
    }

    internal func setTestController(_ testController: TestController?) {
        self.testController = testController
    }

    internal func notifyProcessorReady(_ lgraph: LGraph, _ processor: Any) {
        // TestController notification — stubbed out since TestController protocol
        // does not define these methods in the Swift codebase
    }

    internal func notifyProcessorFinished(_ lgraph: LGraph, _ processor: Any) {
        // TestController notification — stubbed out
    }

    // MARK: - Actual Layout

    internal func layout(_ lgraph: LGraph, _ monitor: IElkProgressMonitor) {
    ...
    internal func layoutTest(_ lgraphs: [LGraph], _ processor: AnyGraphProcessor) {
    ...
    internal func resizeGraph(_ lgraph: LGraph) {
    ...
    internal func resizeGraphNoReallyIMeanIt(_ lgraph: LGraph, _ oldSize: KVector, _ newSize: KVector) {
