...
internal final class org_eclipse_elk_alg_layered_p5edges_orthogonal_OrthogonalRoutingGenerator {

    // MARK: - Constants

    /// differences below this tolerance value are treated as zero.
    internal static let TOLERANCE: Double = 1e-3

    /// a special return value used by the conflict counting method.
    internal static let CRITICAL_CONFLICTS_DETECTED: Int = -1

    /// factor for edge spacing used to determine the conflictThreshold (determined experimentally).
    internal static let CONFLICT_THRESHOLD_FACTOR: Double = 0.5
    /// factor to compute criticalConflictThreshold (determined experimentally).
    internal static let CRITICAL_CONFLICT_THRESHOLD_FACTOR: Double = 0.2

    /// weight penalty for (non-critical) conflicts.
    internal static let CONFLICT_PENALTY: Int = 1
    /// weight penalty for crossings.
    internal static let CROSSING_PENALTY: Int = 16

    /// we'll be using this thing to split hyper edge segments, if necessary.
    internal var segmentSplitter: org_eclipse_elk_alg_layered_p5edges_orthogonal_HyperEdgeSegmentSplitter?
    ...
    @discardableResult
    internal func createDependencyIfNecessary(
    ...
    internal func countConflicts(_ posis1: [Double], _ posis2: [Double]) -> Int {
        var conflicts = 0
    ...
    internal static func countCrossings(_ posis: [Double], _ start: Double, _ end: Double) -> Int {
        var crossings = 0
        for pos in posis {
            if pos > end {
                break
    ...
    internal static func breakNonCriticalCycles(
        _ edgeSegments: [org_eclipse_elk_alg_layered_p5edges_orthogonal_HyperEdgeSegment]
    ) {
        let cycleDependencies =
