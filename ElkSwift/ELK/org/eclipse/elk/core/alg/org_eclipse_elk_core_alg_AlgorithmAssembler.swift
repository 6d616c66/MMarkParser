...
internal class AlgorithmAssembler<P: Hashable & CaseIterable & RawRepresentable, G> where P.RawValue == Int {

    internal var enableCaching: Bool = true
    internal var failOnMissingPhase: Bool = true
    internal var processorComparator: EnumBasedFactory = EnumBasedFactory()

    internal let phasesEnumClass: P.Type
    internal let numberOfPhases: Int
    internal var configuredPhases: Set<P> = []
    internal let additionalProcessors: LayoutProcessorConfiguration<P, G>
    ...
    internal var phaseFactories: [Int: Any] = [:]
    internal var phaseFactoryList: [Any?] = []

    // MARK: - Creation

    internal static func create(_ phaseEnum: P.Type) -> AlgorithmAssembler<P, G> {
    ...
    @discardableResult
    internal func withCaching(_ enabled: Bool) -> AlgorithmAssembler<P, G> {
        enableCaching = enabled
        return self
    }

    @discardableResult
    internal func withFailOnMissingPhase(_ fail: Bool) -> AlgorithmAssembler<P, G> {
        failOnMissingPhase = fail
        return self
    }

    @discardableResult
    internal func withProcessorComparator(_ comparator: EnumBasedFactory) -> AlgorithmAssembler<P, G> {
    ...
    @discardableResult
    internal func clearCache() -> AlgorithmAssembler<P, G> {
        cache.removeAll()
        return self
    }

    @discardableResult
    internal func reset() -> AlgorithmAssembler<P, G> {
        phaseFactoryList = Array(repeating: nil, count: numberOfPhases)
    ...
    @discardableResult
    internal func setPhase(_ phase: P, _ phaseFactory: Any) -> AlgorithmAssembler<P, G> {
    ...
    @discardableResult
    internal func addProcessorConfiguration(_ config: LayoutProcessorConfiguration<P, G>) -> AlgorithmAssembler<P, G> {
    ...
    internal func build(_ graph: G) -> [AnyGraphProcessor] {
    ...
    internal func retrieveProcessorsFromFactories(_ factories: [any ILayoutProcessorFactory]) -> [AnyGraphProcessor] {
...
internal protocol AnyLayoutProcessorFactoryBox {
    func createErased() -> Any
}

/// Type-erased protocol for layout phases
internal protocol AnyLayoutPhaseBox {
...
extension ILayoutPhaseFactory {
    internal func createErased() -> Any {
...
extension org_eclipse_elk_alg_layered_p1cycles_GreedyCycleBreaker: AnyLayoutPhaseBox {
    internal func getLayoutProcessorConfigurationErased(_ graph: Any) -> Any? {
