 ...
 */
internal final class ElkRectangle: Hashable {
     ...
     */
    internal init(_ x: Double, _ y: Double, _ w: Double, _ h: Double) {
        self.x = x
        self.y = y
        self.width = w
        self.height = h
    ...
    internal convenience init(x: Double, y: Double, width: Double, height: Double) {
     ...
     */
    internal init(_ rect: ElkRectangle) {
        self.x = rect.x
        self.y = rect.y
     ...
     */
    internal func setRect(_ nx: Double, _ ny: Double, _ nw: Double, _ nh: Double) {
        self.x = nx
        self.y = ny
        self.width = nw
        self.height = nh
     ...
     */
    internal func getTopLeft() -> KVector {
        return getPosition()
    }
     ...
     */
    internal func getTopRight() -> KVector {
     ...
     */
    internal func getBottomLeft() -> KVector {
     ...
     */
    internal func getBottomRight() -> KVector {
     ...
     */
    internal func getCenter() -> KVector {
     ...
     */
    internal func union(_ other: ElkRectangle) {
     ...
     */
    internal func move(_ offset: KVector) {
        x += offset.x
        y += offset.y
    }

    /// Labeled overload for move(by:).
    internal func move(by offset: KVector) {
        move(offset)
    }
    
    /// Computed origin for compatibility with code using `.origin.x` / `.origin.y`
    internal var origin: KVector {
    ...
    internal static func == (lhs: ElkRectangle, rhs: ElkRectangle) -> Bool {
     ...
     */
    internal func intersects(_ rect: ElkRectangle) -> Bool {
        let r1x1 = self.x
        let r1y1 = self.y
